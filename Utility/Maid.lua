...
-- Useful for encapsulating state and making destructors easy
-- @classmod Maid
-- @see Signal
...
function Maid:__newindex(index, newTask)
    ...
    -- Cleanup old task properly
    if oldTask then
        if type(oldTask) == funcStr then
            oldTask()
        elseif typeof(oldTask) == "RBXScriptConnection" then
            oldTask:Disconnect()
        elseif typeof(oldTask) == "Instance" and oldTask.Destroy then
            oldTask:Destroy()
        ...
        elseif Signal.isSignal(oldTask) then
            oldTask:Destroy()
        elseif typeof(oldTask) == threadStr then
            task.cancel(oldTask)
        end
    end
end
...
function Maid:DoCleaning()
    ...
    while taskData ~= nil do
        ...
        elseif Signal.isSignal(taskData) then
            taskData:Destroy()
        elseif typeof(taskData) == "Instance" then
            if taskData.Destroy then
                taskData:Destroy()
            end
